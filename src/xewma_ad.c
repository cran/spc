...
void xewma_ad
( int *ctyp, double *l, double *c, double *zr, double *mu0, double *mu1,
  int *ltyp, int *r, double *ad)
{
 if (*ctyp==ewma1) *ad = xe1_iglad(*l,*c,*zr,*mu0,*mu1,*r);
 if (*ctyp==ewma2 && *ltyp==fix) *ad = xe2_iglad(*l,*c,*mu0,*mu1,*r);
 if (*ctyp==ewma2 && *ltyp>fix) 
                   *ad = xe2_arlm(*l,*c,0.,200,*mu0,*mu1,*ltyp,*r,5000);
} 
