...
void cewma_crit_be
(int *ctyp, int *design, int *mcdesign, int *rando, double *lambda, double *L0, double *AU, double *mu0, double *z0, int *N, int *jmax, double *AA)
{ int result=0;
  double lAL, lAU, lgL, lgU;  
 *AA = -1.;
 if ( *ctyp==upper ) {
   if ( *mcdesign==classic )  *AA = cewma_U_crit(*lambda, *L0, *mu0, *z0, *N, *jmax);
   if ( *mcdesign==transfer ) *AA = cewma_U_crit_new(*lambda, *L0, *mu0, *z0, *N, *jmax);
 }
 if ( *ctyp==lower ) {
   if ( *mcdesign==classic )  *AA = cewma_L_crit(*lambda, *L0, *AU, *mu0, *z0, *N, *jmax);
   if ( *mcdesign==transfer ) *AA = cewma_L_crit_new(*lambda, *L0, *AU, *mu0, *z0, *N, *jmax);
 }
 if ( *ctyp==two ) {
   if ( *design==sym ) {
     if ( *mcdesign==classic )  *AA = cewma_2_crit_sym(*lambda, *L0, *mu0, *z0, *N, *jmax);
     if ( *mcdesign==transfer ) *AA = cewma_2_crit_sym_new(*lambda, *L0, *mu0, *z0, *N, *jmax);
   }
   if ( *design==unb ) {
     if ( *rando==0 ) {  
       if ( *mcdesign==classic )  result = cewma_2_crit_unb(*lambda, *L0, *mu0, *z0, *N, *jmax, &lAL, &lAU);
       if ( *mcdesign==transfer ) result = cewma_2_crit_unb_new(*lambda, *L0, *mu0, *z0, *N, *jmax, &lAL, &lAU);
       AA[0] = lAL;
       AA[1] = lAU;
     }
     if ( *rando==1 ) {  
       /*if ( *mcdesign==classic )  result = cewma_2_crit_unb(*lambda, *L0, *mu0, *z0, *N, *jmax, &lAL, &lAU);*/
       if ( *mcdesign==transfer ) result = cewma_2_crit_unb_rando_new(*lambda, *L0, *mu0, *z0, *N, *jmax, &lAL, &lAU, &lgL, &lgU);
       AA[0] = lAL;
       AA[1] = lAU;
       AA[2] = lgL;
       AA[3] = lgU;
